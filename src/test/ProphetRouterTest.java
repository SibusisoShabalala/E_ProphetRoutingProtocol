...
import routing.E_ProphetRouter;
import core.Message;
...
public class ProphetRouterTest extends AbstractRouterTest {
	...
	public void setUp() throws Exception {
		...
		ts.putSetting(E_ProphetRouter.PROPHET_NS + "." + 
				E_ProphetRouter.SECONDS_IN_UNIT_S , SECONDS_IN_TIME_UNIT+"");
		setRouterProto(new E_ProphetRouter(ts));
		super.setUp();
	}
	...
	public void testRouting() {
		...
		E_ProphetRouter r4 = (E_ProphetRouter)h4.getRouter();
		E_ProphetRouter r5 = (E_ProphetRouter)h5.getRouter();
		
		checkCreates(6);
		
		h4.connect(h5);
		assertEquals(E_ProphetRouter.P_INIT, r4.getPredFor(h5));
		assertEquals(E_ProphetRouter.P_INIT, r5.getPredFor(h4));
	...
	public void testAging() {
		...
		double newPred = E_ProphetRouter.P_INIT * Math.pow(E_ProphetRouter.GAMMA,2);
		
		assertEquals(newPred, r4.getPredFor(h5));
		assertEquals(newPred, r5.getPredFor(h4));
		
		clock.advance(SECONDS_IN_TIME_UNIT / 10);
		newPred = newPred *	Math.pow(E_ProphetRouter.GAMMA, 1.0/10);
